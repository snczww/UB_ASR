...
def calculate_line_wer_by_list(ground_truth_lines, candidate_lines, tokenizer_model_path, fixed_annotations, decimal_places=3):
    """
    Calculate line-by-line WER for each line between ground truth and candidate lines.
    ...
    if min_length > 10:
        with concurrent.futures.ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
            wer_list = list(executor.map(
                lambda pair: word_list_error_rate(pair[0], pair[1]),
                zip(ground_truth_tokens_list, candidate_tokens_list)
            ))
    else:
        wer_list = [
            word_list_error_rate(gt_tokens, cand_tokens)
            for gt_tokens, cand_tokens in zip(ground_truth_tokens_list, candidate_tokens_list)
        ]
